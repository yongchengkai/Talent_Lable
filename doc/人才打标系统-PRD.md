...
| 发布状态 | 业务含义 | 可编辑 | 可删除 |
|---------|---------|--------|--------|
| 未发布 | 新建草稿或已撤销发布，可自由编辑 | ✓ | ✓ |
| 已发布（未被正式任务使用） | 发布后未被正式任务运行成功或运行中 | ✓ | ✗（需先撤销） |
| 已发布（被正式任务使用） | 被正式任务运行成功或运行中 | ✗（需先撤销任务或复制为新规则） | ✗（需先撤销） |

#### 治理规则

- 编辑权限由正式任务使用情况决定：规则未被任何正式打标任务运行成功或运行中时可自由编辑（无论发布状态），被正式任务使用的规则需先撤销相关任务后再编辑
- 删除权限由发布状态决定：未发布的规则可删除，已发布的规则不可删除
