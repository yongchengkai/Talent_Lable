...
- 没有被任何规则引用的标签可以直接删除，无论启用或停用状态
- 被规则引用的标签不能删除，删除按钮置灰并提示引用数量
- 删除前系统自动检查规则引用，有引用则阻断
...
- 被已发布规则引用的标签不能停用（生产环境依赖该标签，停用会造成"标签停用但规则还在产出结果"的矛盾）
- 只被未发布规则引用的标签可以停用（未发布规则还在调试阶段，用户可以去调整 DSL）
- 没有规则引用的标签可以停用
...
- 撤销发布 → 规则退出生产，不再参与后续打标，但历史结果和证据保留
...
| 操作 | 条件 | 说明 |
|------|------|------|
| 编辑 | 未发布 | 已发布的规则不能改，只能复制出新的来改 |
| 删除 | 未发布 | 已发布或已运行的规则有历史结果和证据链关联，不允许删除 |
| 发布 | 未发布 | 已发布的不需要重复发布 |
| 撤销发布 | 已发布 | 撤销后规则不再参与正式打标 |
| 复制 | 任意 | 打开新建弹窗，预填 DSL、规则解释等内容，编码和名称需重新填写 |
| 模拟打标 | 任意 | 模拟不影响正式打标状态，不写入正式结果 |

#### 状态组合

| 发布状态 | 正式打标状态 | 业务含义 | 可编辑 | 可删除 |
|---------|------------|---------|--------|--------|
| 未发布 | —（无） | 新建草稿，还在编辑中 | ✓ | ✓ |
| 已发布 | 未运行 | 刚发布，还没被正式打标执行过 | ✗ | ✗ |
| 已发布 | 运行成功 | 正式打标执行成功 | ✗ | ✗ |
| 已发布 | 运行失败 | 正式打标执行失败，需排查 | ✗ | ✗ |
| 未发布 | 运行成功 | 曾经上线运行过，现已撤销发布 | ✗ | ✗ |

#### 治理规则

- 只有未发布的规则允许编辑和删除
- 已发布或已运行的规则变更时，必须复制后再修订
- 已运行规则不允许删除，因为有历史标签结果和证据链关联（`EmployeeTagResult` 和 `EmployeeTagResultDetail` 通过 `ruleId` 引用），删除会导致历史证据无法追溯
- 发布状态可自由切换，不影响历史数据
- 如需清理长期废弃的已运行规则，后续迭代可引入"归档"功能，从列表隐藏但数据保留
...
模拟打标和正式打标页面的总数、成功、失败三列数字均可点击：

- 点击后弹出"规则执行明细"弹窗
- 弹窗顶部提供全部/成功/失败三个筛选 Tag
- 表格展示：规则名称、规则编码、规则类型、执行状态、命中人数、错误信息
- 任何一条规则执行失败，整个任务状态即为运行失败
...
模拟打标和正式打标任务的编辑操作遵循以下规则：

| 任务状态 | 可编辑 | 说明 |
|---------|--------|------|
| INIT（待运行） | ✓ | 尚未运行，可修改任务名称和关联规则 |
| FAILED（运行失败） | ✓ | 运行失败，可修改后重新运行 |
| RUNNING（运行中） | ✗ | 正在执行，不可编辑 |
| SUCCESS（运行成功） | ✗ | 已产出结果，不可编辑 |

编辑时自动加载该任务已关联的规则，回填到规则选择器中。
...
| 操作 | 显示条件 | 交互要求 |
|---|---|---|
| 编辑 | runStatus = NOT_RUN | 已运行的规则点击编辑时提示「已运行的规则不可编辑，请复制后修订」 |
...
| 删除 | runStatus = NOT_RUN | Popconfirm 确认；已运行的规则不显示删除按钮 |
